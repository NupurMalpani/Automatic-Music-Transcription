...
class DriveData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        features = [deepcopy([]) for x in range( num_frames)]
        # frames_features = {}
...
train_loader = DataLoader(train_dataset, shuffle=True, collate_fn=mycollate, num_workers=2, batch_size=2,
                          pin_memory=True)
# print(train_loader)
...
class Net(nn.Module):
    ...
    def forward(self, image,feat, diff, hidden,frames):
        ...
        for x in range(frames):
            ...
            out2 = feat[x]
            curr = torch.cat((out.data,out2.data))
            final = torch.cat((final,curr))
        final = Variable(final)
        x, hidden = self.rnn1(final.cuda(), hidden)
