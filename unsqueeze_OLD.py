...
def mycollate(batch):
    ...
    # print(lengths)
    # image_tensors = pack_padded_sequence(Variable(image_tensors), Variable(torch.from_numpy(lengths)), batch_first=True)
    # norm_features = pack_padded_sequence(Variable(norm_features),Variable(torch.from_numpy(lengths)),batch_first=True)
    new_l = torch.LongTensor(batch_size, 1)
    torch.cat(labels, out=new_l)
    # print(image_tensors.type())
    # new_l= new_l.view(batch_size,4)
    return (Variable(image_tensors),Variable(norm_features), new_l,frames,Variable(torch.from_numpy(lengths)))
...
def training(epochs, net, loader):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(loader):
            print("loop", i + 1)
            # print(len(data))
            output_images,output_norm_features,labels,frames,lengths = data
            # imagetensors = data[0]
            # feature_tensors = data[1]
            # labels=  data[2]
            print('in loop')
            # get the inputs
